...
BEGIN
    ...
    IF in_doctype = 'F' then
                ...
                when 'D' then
					begin 
						delete from full_time where hid=in_hospitalid and did=in_searchdoctor;
                        delete from hospital_doctor where hid=in_hospitalid and did=in_searchdoctor;
					end;
                else set result = -1;
                END case;
	...
	ELSEIF in_doctype = 'C' then
                ...
                when 'D' then 
					begin 
						delete from consulting where hid=in_hospitalid and did=in_searchdoctor;
                        delete from hospital_doctor where hid=in_hospitalid and did=in_searchdoctor;
					end;
                else set result = -1;
                END case;
